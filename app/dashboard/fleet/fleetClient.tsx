...
export default function FleetClient({
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteDriver(deleteModalId)

      setDrivers((prev) =>
        prev.filter((d) => d.id !== deleteModalId)
      )

      showSuccess("Chauffeur supprimé")
    } catch {
  ...
  async function handleChangeStatus(id:string,status:string){
    ...
    try{
      const updated = await updateDriverStatus(id, status as 'ACTIVE' | 'IDLE' | 'MAINTENANCE')
                          ...
                          <div className="absolute right-0 top-full mt-1 z-50 min-w-[220px] py-1 bg-surface border border-border rounded-xl shadow-xl animate-in fade-in zoom-in-95 duration-150">
                            ...
                            <div className="px-4 py-2 border-t border-border mt-1">
                              ...
                              <select
                                ...
                                className="w-full bg-surface border border-border rounded-lg py-2  text-sm text-text-main focus:ring-2 focus:ring-primary outline-none"
                              >
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    const updated = await updateDriver(editDriverId, {
                      name: editName.trim() || undefined,
                      email: editEmail.trim() || undefined,
                      phone: editPhone.trim() || null,
                      role: editRole.trim() || null,
                      status: editStatus as 'ACTIVE' | 'IDLE' | 'MAINTENANCE' || undefined,
                        ...
                        d.id === editDriverId
                          ? {
                              ...
                              licenseExpiry: updated.licenseExpiry
                                ? String(updated.licenseExpiry).slice(0,10)
                                : d.licenseExpiry,
                            }
                          : d
                      )
                    ...
                    console.error(err)
                    const errMsg = "Erreur lors de la mise à jour"
